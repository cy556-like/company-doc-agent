...
write_file(r"app\utils\pdf_generator.py", '''\
"""
PDF 生成工具模块
"""
import os
...
# ========== 2. document.py - 关键修复：延迟导入 ChromaDB ==========
write_file(r"app\rag\document.py", '''\
"""
文档处理与向量化模块 (RAG)
修复：延迟导入 ChromaDB，即使 ChromaDB 损坏也不影响文档读取
"""
import os
...
# ========== 3. prompts.py ==========
write_file(r"app\agent\prompts.py", '''\
"""
Prompt 模板模块
"""
...
TOOL_DESCRIPTIONS = {
    "search_documents": "在公司文档库中搜索与查询相关的文档内容。",
    "lookup_employee": "查询公司员工信息。",
    "list_documents": "列出知识库中所有可用的文档。",
...
# ========== 4. routes.py ==========
write_file(r"app\api\routes.py", '''\
"""
FastAPI 路由定义
"""
import os
...
async def modify_document(
    ...
    try:
        ...
        system_prompt = """你是一个文档修改助手。按照修改要求修改文档，返回修改后的完整内容。
规则：
1. 只返回修改后的文档内容
2. 保持原文档格式和结构
3. 只修改用户要求的部分
...
# ========== 5. tools.py ==========
write_file(r"app\agent\tools.py", '''\
"""
Agent 工具定义模块
"""
import json
...
# 关键修复：把 sendMessage 函数中的 [修改文档] 逻辑替换为 /chat-with-file
# 精确定位并替换整个 if (selectedFile && message) 分支

import re

# 找到 sendMessage 中 if (selectedFile && message) 的代码块
# 策略：替换关键行

# 1. 替换显示文本
html = html.replace(
    "addMessageToUI('user', `[修改文档] ${selectedFile.name}",
    "addMessageToUI('user', `📎 ${selectedFile.name}"
)
...
# 3. 替换返回处理逻辑
# 旧逻辑：
old_response = """let content = data.message || '修改完成';
...
html = html.replace(old_response, new_response)

# 4. 更新 placeholder
html = html.replace(
    'placeholder="输入问题，或选择文档后输入修改要求..."',
    'placeholder="输入问题..."'
)

# 5. 选文件后更新 placeholder
if "针对此文件" not in html:
    html = html.replace(
        "document.getElementById('fileBar').style.display = 'flex';\n    }",
        "document.getElementById('fileBar').style.display = 'flex';\n        document.getElementById('msgInput').placeholder = '针对此文件输入问题，或修改要求...';\n    }"
    )

# 删掉所有文件后恢复 placeholder
if "removeFile" in html and "输入问题..." not in html[html.find("function removeFile"):html.find("function removeFile")+500]:
    html = html.replace(
        "document.getElementById('fileBar').style.display = 'none';\n}",
        "document.getElementById('fileBar').style.display = 'none';\n    document.getElementById('msgInput').placeholder = '输入问题...';\n}"
    )
...
if "fpdf2" not in req:
    with open(req_path, "a", encoding="utf-8") as f:
        f.write("\nfpdf2>=2.7.0\n")
    print("  ✅ requirements.txt 添加 fpdf2")
else:
    print("  ✅ requirements.txt 已包含 fpdf2")
